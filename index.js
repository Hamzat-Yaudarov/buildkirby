        ...
        reply_markup: {
            inline_keyboard: [
                ...
                [
                    ...
                    { text: '🎰 Управление ��отереями', callback_data: 'admin_lottery' }
                ],
                [
...
💡 Введите тольк�� число (например: 26)`, {
                    parse_mode: 'Markdown',
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🔄 Новый пример', callback_data: 'new_captcha' }]
...
Выберите действие из меню ниже:`;
        ...
        bot.sendMessage(chatId, '❌ произошла ошибка. Попр����буйте позже.');
    }
});
...
bot.onText(/\/audit_referrals/, async (msg) => {
    ...
    try {
        ...
        message += `⚠️ Требуют корректировки: ${summary.totalUsersAffected}\n`;
...
bot.onText(/\/endlottery (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (lotteryResult.rows.length === 0) {
            bot.sendMessage(chatId, `❌ Активная лотерея с ID ${lotteryId} не найден��.`);
            return;
        }
...
• Билеты: автоматически за каждого нового реферала`, { parse_mode: 'Markdown' });
        ...
        if (prizes.length === 0) {
            bot.sendMessage(chatId, '❌ Необход��мо указать хотя бы ��дин приз!');
            return;
        }
...
Выберите действие:`;
            ...
            // Stars Agent функциона������ность уд��лена - только ручная обработка заявок
        ...
        if (error.message && error.message.includes('query is too old')) {
            console.log('[CALLBACK] Ignoring old callback query');
            return; // Just ignore old queries
        }

        // Try to respond to the callback query if possible
        try {
            await bot.answerCallbackQuery(callbackQuery.id, {
            ...
            console.error('Could not answer callback query:', answerError.message);
        }
    }
});
...
⚠️ **Важно:** Рефе��ал засчитывается только после подписки на все каналы!`;

    const keyboard = {
        reply_markup: {
            inline_keyboard: [
                [{ text: '📥 п����елиться', switch_inline_query: `Присоединяйся к боту для заработка зв��зд! ${inviteLink}` }],
                [{ text: '🏠 В главное меню', callback_data: 'main_menu' }]
            ]
        }
    };
...
📊 **Сег����ня кликнуто:** ${currentClicks}/10
...
⏳ **До с��едующего клика:** ${minutesLeft} мин
...
💎 **Баланс по��ле вывода:** ${(parseFloat(user.balance) - amount).toFixed(2)} ⭐`;
            ...
            await bot.editMessageText('❌ Оши��ка обработки заявки. Попробуйте позже.', {
                chat_id: chatId,
                message_id: messageId,
                ...getBackToMainKeyboard()
            });
...
Если у вас есть во��росы, обратитесь к администрации.`;
...
bot.onText(/\/delete_lottery (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (hasTickets) {
            bot.sendMessage(chatId, `❌ Нельзя удалить лотерею с ID ${lotteryId} - в ней есть участники! Сначала завершите лотерею ��омандой /endlottery ${lotteryId}`);
            return;
        }
        ...
        if (result.rowCount > 0) {
            bot.sendMessage(chatId, `✅ Лотерея �� ID ${lotteryId} у��алена!`);
        } else {
...
bot.onText(/\/weekly_rewards_status/, async (msg) => {
    ...
    try {
        ...
        message += `🔄 **Автоматичес��ие награды:** ${settings.auto_rewards_enabled ? '✅ Включе��ы' : '❌ Отключены'}\n`;
...
⚠️ **ОСТОРОЖНО:** Высокие лимит�� увеличивают риск блокировки!
...
• Агре��сивные: 300/день, 30/час, 100 за раз`;
        ...
        if (hourLimit > dayLimit) {
            bot.sendMessage(chatId, '❌ Часовой лимит не мож��т быть больше дневного.');
            return;
        }
...
async function handleSubscriptionStats(chatId, messageId) {
    try {
        ...
        message += `• Всего уникал��ных пользователей: **${uniqueUsersCount}**\n`;
        message += `• Активных каналов: **${stats.filter(s => s.is_active).length}**\n`;
        message += `• Всего канало��: **${stats.length}**\n\n`;

        message += `💡 **Как работае��:**\nКаждый пользователь может увели��ить счетчик только один раз - при первой успешной проверке подписки. Повторные проверки того же пользователя не увеличивают счётчик.`;
...
async function handleUniqueUsers(chatId, messageId) {
    try {
        ...
        message += `💡 **Пояснение:**\nКаждый пользователь учитывается в стати��тике только один раз - при первой успешной проверке подписки. Повторные проверки этого же пользователя не увеличивают счётчик.`;
...
async function handleSubscriptionHistory(chatId, messageId) {
    try {
        ...
        for (const check of history) {
            ...
            message += `   ⏰ ${checkTime}\n`;
            message += `   📺 Акт��вных каналов: ${channelsCount}\n\n`;
        }
...
async function handleSubGramCheck(chatId, messageId, userId) {
    try {
        ...
        await bot.editMessageText('❌ Ошибка про��ерки спонсорских каналов.', {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: {
                inline_keyboard: [
...
async function handleSubGramGender(chatId, messageId, userId, gender) {
    try {
        ...
        if (!genderResponse.success) {
            await bot.editMessageText('❌ Ошибка получения каналов с указанным полом.', {
                chat_id: chatId,
                message_id: messageId,
                reply_markup: {
                    inline_keyboard: [
...
async function handleAdminSubGram(chatId, messageId) {
    try {
        ...
        await bot.editMessageText('❌ Ошибка загрузки SubGram управления.', {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: { inline_keyboard: [[{ text: '🔙 Назад', callback_data: 'admin_menu' }]] }
        });
