...
��� Пр��должайте приглашать др��зей и зарабатывайте еще больше звёзд!`;
...
⚙️ **Обработка:** ${status.processing ? 'Ак��ив��а' : 'Неактивна'}
...
✅ Если вы видите это сообщени�� - работает НОВА�� версия!
...
bot.onText(/\/test_my_captcha/, async (msg) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, '✅ Ваша кап������ сброшена для тестирования. Нажмите /start для прохождения капчи.');
    } catch (error) {
...
bot.onText(/\/endlottery (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (participantCount === 0) {
            bot.sendMessage(chatId, `❌ В лотере�� ${lottery.name} нет участников!`);
            return;
        }
        ...
        if (result.rowCount > 0) {
            ...
            try {
                ...
                if (qualificationResult.qualified && qualificationResult.processed) {
                    ...
                    message += `\n✅ Пользователь квалифициров��н (все условия выполнены)`;
                } else {
                    message += `\n⏳ Пользователь пока не квалифициров��н (нужны: к��пча + подписка + 1 реферал)`;
                }
...
bot.onText(/\/starsupplayer (\d+) (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (result) {
            bot.sendMessage(chatId, `✅ Поль��ователю ${targetUserId} д��бавлено ${starsCount} ⭐!`);
        ...
        bot.sendMessage(chatId, '❌ Ошибк�� добавления зв��зд.');
    }
});
...
📈 **/tracking_stats ID** - статистика сс��лки
...
Используйте:
\`/create_auto_referral_lottery ��азвание|время_часов|место1:приз1|место2:приз2|...\`
...
bot.onText(/\/unique_users/, async (msg) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, '❌ Ошибка получения данных о по���ьзо��ателях.');
    }
});
...
bot.onText(/\/create_promo (.+)/, async (msg, match) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, '❌ Ошибка созда��ия п��ом��кода (возможно, код уже существует).');
    }
});
...
💡 ��ведите только число (например: 26)`, {
            ...
            case 'restart_after_captcha':
                ...
                await bot.editMessageText('�� Пер��з��пуск...', {
                    chat_id: chatId,
                    message_id: msg.message_id
                });
...
👤 Приглашённый вами польз��ватель **${userInfo.first_name}** выполнил все условия:
✅ Прошёл капчу
✅ Подписался на все каналы
��� Пригласил своего первого реферала
...
👥 Продолжайте приглашать друзей и зарабатывайте еще бо��ьше звёзд!`;
...
🎯 Теперь этот реферал за��читывается полно��тью!`;
            ...
            case 'admin_weekly_enable':
                if (isAdmin(userId)) {
                    ...
                    await bot.answerCallbackQuery(callbackQuery.id, { text: '❌ У вас нет прав до��тупа!', show_alert: true });
                }
                break;
...
• ��� **Зад��н��я** - выполняйте задачи за во��награжд��ние
...
💰 **З��раб��тывай��е вместе с друзьями!**
Приг��ашайте друзей и получайте **3 ⭐** за каждого нового пользователя!
...
⚠️ **Важно:** Рефе��ал засчитывается только после по��п����ски на все каналы!`;
...
🎁 **Завтра дост��пно:** 10 ���овых кликов
...
⌛ **Время ���жидания:** ${delayMinutes} мин (увеличивается с к��ждым кли��ом)`;
 ...
 **��овет:** С ���аждым кликом время ожид���ния увеличивается на 5 минут`;
    ...
    try {
        ...
        try {
            ...
            if (parseFloat(currentUser.rows[0].balance) < amount) {
                await db.executeQuery('ROLLBACK');
                await bot.editMessageText('❌ Недостаточно звёзд для вы��ода!', {
                    chat_id: chatId,
                    message_id: messageId,
                    ...getBackToMainKeyboard()
                });
...
💎 **Баланс по��ле вывода:** ${(parseFloat(user.balance) - amount).toFixed(2)} ⭐`;
            ...
            // ОСТАВЛЯЕМ: Отправка уведомления в админский кана��
            try {
                ...
                // ��аявка уже создана и средства списаны
            }

            // Уведомление поль��ователя об успехе
            await bot.editMessageText('✅ Заявка на вы��од отправлена! Ожидайте обработки.', {
                chat_id: chatId,
                message_id: messageId,
                ...getBackToMainKeyboard()
            });
...
1��⃣ **Кликер** - нажимайте к���ждый день и получайт�� 0.1 ⭐
...
async function handleRatingsWeek(chatId, messageId) {
    try {
        ...
        await bot.editMessageText(message, {
            chat_id: chatId,
            message_id: messageId,
            parse_mode: undefined, // Убираем Markdown для безопасн��сти
            ...getBackToMainKeyboard()
        });
    } catch (error) {
...
async function handleLottery(chatId, messageId, userId = null) {
    try {
        ...
        keyboards.push([{ text: '◀️ В ��л��вное меню', callback_data: 'main_menu' }]);
...
🎯 Теперь этот р��ферал засчитывается полностью!`;
...
🎉 Теперь в�� можете пользоваться б��том! Нажмите /start для продолжения.`, {
                        parse_mode: 'Markdown',
                        reply_markup: {
                            inline_keyboard: [
            ...
            if (user && user.temp_action) {
                if (user.temp_action === 'awaiting_promocode') {
                    ...
                    if (success) {
                        ...
                        bot.sendMessage(chatId, '❌ Пр��мокод уже использов��н или неде��ствителен!');
                    }
...
Если у вас ест�� во��росы, обр��титесь к а��министрации.`;
...
💸 Средства возвращен�� на баланс.`, { parse_mode: 'Markdown' });
...
async function executeRejectAllWithdrawals(chatId, messageId, adminId, reason) {
    try {
        ...
        if (result.success) {
            ...
            for (let i = 0; i < pendingWithdrawals.length; i++) {
                ...
                try {
                    await sendThrottledMessage(withdrawal.user_id,
                        ...
                        `📅 **Дата по��ачи:** ${new Date(withdrawal.created_at).toLocaleDateString('ru-RU')}\n` +
                        `📝 **Причина:** ${reason}\n\n` +
            ...
            successMessage += `• Отклонено заявок: ${result.count}\n`;
            ...
            await bot.editMessageText(`❌ **Ошибка при отклонении заявок**\n\n${result.message || 'Неизвест��ая ошибка'}`, {
                chat_id: chatId,
                message_id: messageId,
                parse_mode: 'Markdown',
...
bot.onText(/\/create_tracking_link (.+)/, async (msg, match) => {
    ...
    try {
        ...
        if (!linkName) {
            bot.sendMessage(chatId, '❌ укажите название ссылки! Используйте: /create_tracking_link Название_р��кла��ы');
            return;
        }
...
📋 **Все ссылки:** /list_tracking`;
        ...
        bot.sendMessage(chatId, `❌ ��шибка создания ссылки: ${error.message}`);
    }
});
...
bot.onText(/\/delete_lottery (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (hasTickets) {
            bot.sendMessage(chatId, `❌ Нельз�� удалить лотерею с ID ${lotteryId} - в ней ест�� уча��тники! ��начала завершите лотерею ��омандо�� /endlottery ${lotteryId}`);
            return;
        }
    ...
    try {
        ...
        for (let i = 0; i < users.rows.length; i++) {
            ...
            if (i % 10 === 0 || i === users.rows.length - 1) {
                ...
                try {
                    await bot.editMessageText(`📤 **Рассыл��а в процессе...**\n\n👥 Пользователей: ${totalUsers}\n��� Отправлено: ${successCount}\n��� Ошибок: ${failCount}\n⏳ прогресс: ${progress}%`, {
                        chat_id: chatId,
...
⚠️ **Внимание:** Рассылка будет ����прав���ена сразу пос��е получения сообщения!
...
async function distributeWeeklyRewards(isManual = false) {
    ...
    try {
        ...
        let rewardMessage = '🏆 **Еженедельные награды!**\n\n📅 **Топ-5 пользов��тел��й по очкам за неделю:**\n\n';
...
async function handleAdminWeeklyRewards(chatId, messageId) {
    try {
        ...
        const message = `🏆 **Управление недельным�� наградами**
...
5 место: 15 ⭐`;
        ...
        await bot.editMessageText('❌ Ошибка загрузки управления неде��ьными наградами.', {
            chat_id: chatId,
            message_id: messageId,
            ...getBackToMainKeyboard()
        });
        ...
        for (const withdrawal of oldWithdrawals.rows) {
            try {
                ...
                if (withdrawal.type === 'stars' && withdrawal.amount <= 200) {
                    ...
                    const result = { success: false, error: 'Stars Agent отключен, только ��учная обработка' };
...
• Агре��сивные: 300/день, 30/час, 100 за раз`;
        ...
        if (hourLimit > dayLimit) {
            bot.sendMessage(chatId, '❌ Часовой лимит не може�� ��ыть больше дневного.');
            return;
        }
...
# Создать т��блицу настроек если не суще��твует
...
# Обновить или создать н��стройки
cursor.execute('''
...
🤖 **Перез��пустите агент** для применен��я изменений:
...
async function handleSubscriptionStats(chatId, messageId) {
    try {
        ...
        if (stats.length === 0) {
            await bot.editMessageText(`📈 **Статис��ика подписок**\n\n❌ Нет данных о подп��с��ах.\n\nДоб��вьте обязательные каналы и дождитесь первых пр��верок подписок.`, {
                chat_id: chatId,
                message_id: messageId,
                parse_mode: 'Markdown',
                reply_markup: {
                    inline_keyboard: [
                        [{ text: '📺 Управле��ие каналами', callback_data: 'admin_channels' }],
                        [{ text: '🔙 Назад', callback_data: 'admin_menu' }]
                    ]
                }
            });
        ...
        let message = `📈 **Ст��тистика подпис��к ��о каналам**\n\n`;
        ...
        message += `���� **Как работае��:**\nКаждый пользовате��ь может увеличить счетчик только один раз - при первой успешной проверке подписки. Повторные проверки того же пользователя не увеличивают счётчик.`;
...
async function handleUniqueUsers(chatId, messageId) {
    try {
        ...
        await bot.editMessageText('❌ Ошиб��а загрузки да��ных о пользователях.', {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: {
                inline_keyboard: [
...
async function handleEnhancedSubscriptionCheck(chatId, messageId, userId) {
    try {
        ...
        const subscriptionDetails = await checkAllSubscriptionsDetailed(userId, true);
        const allSubscribed = subscriptionDetails.allSubscribed || subscriptionDetails.hasErrors;
...
async function handleSubGramCheck(chatId, messageId, userId) {
    try {
        ...
        if (!checkResponse.success) {
            await bot.editMessageText('❌ Ошибка проверки SubGram кан��лов. Попробу��те позже.', {
                chat_id: chatId,
                message_id: messageId,
                reply_markup: {
                    inline_keyboard: [
