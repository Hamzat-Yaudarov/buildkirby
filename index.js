...
async function getEnhancedSubscriptionMessage(userId) {
    try {
        ...
        if (channelCount === 0) {
            ...
            message += '\n📌 После подписки на в��е каналы нажмите кнопку проверки';
    ...
    return {
        reply_markup: {
            inline_keyboard: [
                ...
                [
                    { text: '⏭️ Пропустить за��ание', callback_data: 'task_skip' },
...
💡 Введите т��л��к�� число (например: 26)`, {
                    parse_mode: 'Markdown',
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🔄 Новый пример', callback_data: 'new_captcha' }]
...
⚙️ **Обработка:** ${status.processing ? 'Актив��а' : 'Неактивна'}
...
bot.onText(/\/starsupplayer (\d+) (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, '❌ Ошибк�� добавления звёзд.');
    }
});
...
bot.onText(/\/admin/, async (msg) => {
    ...
    if (!isAdmin(userId)) {
        bot.sendMessage(chatId, '❌ У вас нет прав до��т��па к панели администратора.');
        return;
    }
...
👥 **/refupplayer [ID] [число]** - добавить рефералов пользов��телю
...
**Призы:**`;
        ...
        message += `\n\n⏰ **��аверше��ие:** ${endsAt.toLocaleString('ru-RU')}`;
...
• Билеты: автоматиче��ки за каждого нового реферала`, { parse_mode: 'Markdown' });
            return;
        }
...
bot.onText(/\/subscription_stats/, async (msg) => {
    ...
    try {
        ...
        let message = `📈 **Статистика подп��сок по кана��ам**\n\n`;
        ...
        for (const stat of stats) {
            ...
            const addedDate = stat.channel_added_at ? new Date(stat.channel_added_at).toLocaleDateString('ru-RU') : 'Не��звестно';
...
bot.onText(/\/create_promo (.+)/, async (msg, match) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, '❌ Ошибка создания п��ом��кода (возможно, код уже существует).');
    }
});
...
💡 Введите только число (например: 26)`, {
                ...
                await bot.answerCallbackQuery(callbackQuery.id, { text: '🔄 Новый пример сгене��иро��ан!' });
                break;
...
• Пригл��шено сегодня: **${user.referrals_today}**
...
⌛ **Время ���жидания:** ${delayMinutes} мин (увеличивается с каждым кли��ом)`;
...
����� **Инструкция:**
1. Н��жмите "Подписат������" для перехода к каналу
...
async function handleTaskCheck(chatId, messageId, userId, taskId) {
    try {
        ...
        try {
            ...
            if (member.status === 'left' || member.status === 'kicked') {
                await bot.editMessageText('��� Вы не подпи��аны на ка��ал! Подпишитесь и попробуйте снова.', {
                    chat_id: chatId,
                    message_id: messageId,
            ...
            if (error.response?.body?.error_code === 400 || error.response?.body?.description?.includes('chat not found')) {
                ...
                await bot.editMessageText('❌ Ош��б��а проверки подписки. Попробуйте позже или обратитесь к адми��истрации.', {
                    chat_id: chatId,
                    message_id: messageId,
                    ...getBackToMainKeyboard()
                });
...
async function handleLottery(chatId, messageId, userId = null) {
    try {
        ...
        keyboards.push([{ text: '◀️ В гл��вное меню', callback_data: 'main_menu' }]);
...
async function handleWithdrawalApproval(chatId, messageId, callbackData) {
    try {
        ...
        if (!approvedWithdrawalId) {
            await bot.editMessageText('❌ З��явка на вывод не найдена или уже обработана.', {
                chat_id: chatId,
                message_id: messageId
            });
            return;
...
✅ **Ваша заявка на вывод ��добмена!**

💰 **Сумма:** ${typeDisplay}

🎯 **Награда уже выплачена!** Спасибо за использование нашего бот��!

👥 Продолжайте приглашат�� друзей и зарабатыв��ть еще больше!`;
...
🎉 Теперь вы можете пользоваться б��том! Нажмите /start для продолжения.`, {
                        parse_mode: 'Markdown',
                        reply_markup: {
                            inline_keyboard: [
            ...
            if (user && user.temp_action) {
                if (user.temp_action === 'awaiting_promocode') {
                    ...
                    if (success) {
                        ...
                        bot.sendMessage(chatId, '❌ Пр��мокод уже использован или неде��ствителен!');
                    }
            ...
            for (let i = 0; i < pendingWithdrawals.length; i++) {
                ...
                try {
                    await sendThrottledMessage(withdrawal.user_id,
                        ...
                        `📅 **Дата подачи:** ${new Date(withdrawal.created_at).toLocaleDateString('ru-RU')}\n` +
                        `📝 **Причина:** ${reason}\n\n` +
...
bot.onText(/\/delete_lottery (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (hasTickets) {
            bot.sendMessage(chatId, `❌ Нельз�� удалить лотерею с ID ${lotteryId} - в ней ест�� уча��тники! ��начала завершите лотерею ��омандой /endlottery ${lotteryId}`);
            return;
        }
...
async function distributeWeeklyRewards(isManual = false) {
    ...
    try {
        ...
        let rewardMessage = '🏆 **Еженедельные награды!**\n\n📅 **Топ-5 пользовател��й по очкам за неделю:**\n\n';
...
bot.onText(/\/weekly_rewards_status/, async (msg) => {
    ...
    try {
        ...
        message += `🔄 **Автоматичес��ие награды:** ${settings.auto_rewards_enabled ? '✅ Включен��' : '❌ Отключены'}\n`;
        ...
        if (users.length === 0) {
            message += 'Пока нет активных пользо��ателей\n';
        } else {
        ...
        bot.sendMessage(chatId, '❌ Ош��бка получения с��атуса наград.');
    }
});
...
\`/admin\` → \` Stars Agent\` �� \`⏹️ Остановить\` → \`▶️ Запус��ить\``;
            ...
            bot.sendMessage(chatId, '❌ Ошибка обновления лим��тов. Попр��буйте поз��е.');
        }
...
async function handleSubscriptionStats(chatId, messageId) {
    try {
        ...
        message += `👥 **Уникальных пользо��ателей прошло проверку:** ${uniqueUsersCount}\n`;
        message += `🔄 *(Каждый пользователь считается только один раз)*\n\n`;
...
async function handleSubscriptionHistory(chatId, messageId) {
    try {
        ...
        await bot.editMessageText('❌ Ошибка загрузки истории пров��рок.', {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: {
                inline_keyboard: [
