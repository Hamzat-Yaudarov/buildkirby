...
// Защита от спама - храним пос��едний вызов для каждого пользователя
...
// Удаляем предсказуемый счетчик ру��етки
...
function normalizeChannelIdentifier(channelInput) {
    ...
    if (channelInput.startsWith('https://t.me/')) {
        ...
        return {
            ...
            title: username,
            url: channelInput // Оставляем тригинальную ссылку
        };
    }
...
async function checkPersonalChannelsSubscription(userId, skipOnError = false) {
    try {
        ...
        try {
            console.log(`🔄 Fallback: используем каналы ��з конфигурации`);
...
async function checkReferralConditions(userId) {
    try {
        ...
        if (completedTasks < 2) {
            console.log(`👥 Реф��рал ${userId} выполнил только ${completedTasks}/2 заданий`);
            return;
        }
...
// Капча с 8 кнопками (животные и фрукты)
function showCaptcha(chatId, userId) {
    const message = '🤖 Для продолжения работы с ботом пройдите проверку:\n\n' +
                   '🍓 Найдите и на��мите на ЗЕМЛЯНИКУ среди представленных вариантов:';
...
async function handleCaptcha(chatId, userId, data, messageId, callbackQueryId) {
    ...
    if (choice === 6) { // Правильный отве�� - земляника (6-я кнопка)
        ...
        // Теперь переходим к проверке подписки
        ...
        if (lastCheck && (now - lastCheck) < 3000) { // 3 секунды защита
            console.log(`⚠️ Защита от спама: пропускаем повт��рные проверки для пользователя ${userId}`);
            return;
        }
        ...
        if (!subscriptionStatus.isSubscribed) {
            ...
            if (subscriptionStatus.subscriptionData?.links?.length > 0) {
                ...
                if (!messageSent) {
                    console.log(`⚠️ ��е удалось отправить спонсорское сообщение пользователю ${userId} после капчи`);
                }
            } else {
        ...
        console.log(`✅ Пользователь ${userId} подписан на ��СЕ каналы после капчи, показываем главное меню`);

        const successMessage = '✅ Отлично! Вы прошли капчу и подписались на все каналы!\n\n' +
                              '🎉 Добро пожал��вать в бота для заработка звёзд!\n\n' +
        ...
        await bot.editMessageText('❌ Неправильный ответ! Попробуйте еще раз.\n\n' +
                                '🍓 Найдите и нажмите на ЗЕМЛЯНИКУ среди представленных вариантов:', {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: {
                inline_keyboard: [
...
// Проверка п��дпис��и пользов��теля на СПОНСОРСКИЕ каналы (для блокировка доступа к функциям бота)
async function checkUserSubscription(userId, chatId, firstName = '', languageCode = 'ru', isPremium = false) {
    try {
        ...
        if (taskChannels.status === 'error') {
            console.log(`❌ Ошибка SubGram, пытаемс�� получить ссылки альтернативным способом`);
        ...
        // ВАЖНО: статус "warning" означает что пользовате��ь НЕ подписа��!
                ...
                for (let i = 0; i < attempts.length; i++) {
                    try {
                        ...
                        if (linksCheck.links && linksCheck.links.length > 0) {
                            ...
                            console.log(`⚠ Попытка ${i + 1}: status='ok' но нет ссылок (пользо��атель уже подписан?)`);
                        } else {
...
bot.onText(/\/cancel/, async (msg) => {
    ...
    if (broadcastStates.has(userId)) {
        ...
        await bot.sendMessage(chatId, 'ℹ️ Нет активны�� операций для отмены.');
    }
});
...
bot.on('message', async (msg) => {
    ...
    // Проверяем, если админ ожидаем ввода сообщения для рассылки
    ...
    if (broadcastStates.has(userId) && broadcastStates.get(userId).waiting) {
        ...
        // Генерируем уникальный ID для сообщения (исправлено для BUTTON_DATA_INVALID)
        ...
        // Подтверждаем отправку (��гранич���ваем отображение длинных сообщений)
        ...
        const confirmationMessage = `ℹ️ Подтвердите отправку сообщения:\n\n` +
                               `“${messageText}”\n\n` +
                               `⚠️ Сообщени�� будет отправлено всем пользователям бота!`;
...
async function showMainMenu(chatId, userId = null) {
    const message = '1️⃣ Получи свою личную ссылку жми «⭐️ Заработать звезды»\n\n' +
                   '2️👥 Приглашай друзей — 2 ⭐ за каждого!\n\n' +
                   '✅ Дополнительно:\n' +
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        ...
        if (!isAllowedCommand) {
            ...
            // Если подписан на все каналы (SubGram И личные) - разрешаем доступ
        ...
        switch (data) {
            ...
            default:
                if (data.startsWith('withdraw_')) {
                    ...
                    // Удаляем сообщение из временного хранилища
                    pendingBroadcastMessages.delete(messageId);
...
// Проверка подписки в контексты личных каналов (с пропуском ошибок)
async function handleSubscriptionCheckPersonal(chatId, userId, messageId, callbackQueryId = null) {
    ...
    if (subscriptionStatus.isSubscribed || !subscriptionStatus.subscriptionData?.links?.length) {
        // Проверяем личные спонсорские каналы (ВСЕГДА с пропуском о��ибок в этом контексте)
...
async function handleSubscriptionCheck(chatId, userId, messageId, callbackQueryId = null) {
    ...
    // Если нет ссылок для подписки - значит п��льзователь подписан на SubGram каналы
    if (subscriptionStatus.isSubscribed || !subscriptionStatus.subscriptionData?.links?.length) {
        ...
        if (availableCustomTask) {
            ...
            const keyboard = {
                inline_keyboard: [
                    ...
                    [{ text: '🏠 В главное меню', callback_data: 'main_menu' }]
                ].filter(row => row.length > 0) // Убиваем пустые строки
            };
                ...
                if (member.status === 'left' || member.status === 'kicked') {
                    ...
                    const subscriptionKeyboard = {
                        inline_keyboard: [
                            ...
                            [{ text: '✅ Проверить выполнение', callback_data: `check_custom_task_${taskId}` }],
                            [{ text: '🏠 В главное меню', callback_data: 'main_menu' }]
                        ]
                    };
...
async function handleAdminCallback(chatId, userId, data, messageId, callbackQueryId) {
    ...
    switch (data) {
        ...
        case 'admin_sponsor_sync':
            ...
            return; // Не отвечаем на callback здесь, так как функция сама отвечает
    }
...
// Обработчик заявок на вывод с упиранием кнопок и отправкой в платежный чат
async function handleWithdrawalAction(chatId, userId, data, callbackQueryId, messageId) {
    ...
    try {
        ...
        if (action === 'approve') {
            ...
            console.log(`🔄 Возвращаем средства пользователю ${requestData.user_id}, сумма: ${requestData.amount}`);
            await Database.updateUserBalance(requestData.user_id, requestData.amount, 'add');
            console.log(`✅ Средства возвращены успешно`);
            ...
            console.log(`💰 ОТПРАВКА В ПЛАТЕЖНЫЙ ЧАТ (REJECT): ${config.PAYMENTS_CHAT_ID}`);
    ...
    try {
        ...
        for (const user of users.rows) {
            try {
                ...
                // Небольшая задержка ме��ду отправками для избежания лимитов
...
// Показать админ интерфейс спонсорских каналов
...
async function showSponsorChannelsTop(chatId, messageId) {
    try {
        ...
        console.error('Ошибка показа топа каналов:', error);
