...
async function handleRatingsWeek(chatId, messageId) {
    try {
        ...
        if (result.rows.length === 0) {
            message += 'Пока нет ак��ивных пользователей за эту неделю.';
        } else {
            result.rows.forEach((user, index) => {
                const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index + 1}.`;
                const safeName = cleanDisplayText(user.first_name);
...
🤖 **Перез��пустите агент** для применения изменений:
...
async function handleSubGramCheck(chatId, messageId, userId) {
    try {
        ...
        if (!checkResponse.success) {
            await bot.editMessageText('❌ Ошибка проверки SubGram кан��лов. Попробуйте позже.', {
                chat_id: chatId,
                message_id: messageId,
                reply_markup: {
                    inline_keyboard: [
